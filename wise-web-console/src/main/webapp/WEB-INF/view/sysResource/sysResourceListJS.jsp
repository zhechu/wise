...
$(document).ready(function(){
	    ...
	    columns: [{
	        ...
	        formatter:function(value,row,index){
	        	...
	        	<shiro:hasPermission name="sys:resource:update">
	        		operations = '<a href="${ctx }/sysResource/edit?id='+value+'">编辑</a>';
		        </shiro:hasPermission>
	            return operations;
	        }
	    }],
    ...
    $("#delBtn").on("click", function(){
    	var sysResources = $('#resourceTable').bootstrapTable('getSelections');
    	if (sysResources==null || sysResources.length==0) {
    		$("#msg-box").addMsgBox('info', "请选择要删除的数据");
    		return;
    	}
    	var ids = [];
    	$.each(sysResources, function(i, n){
    		ids.push(n.id);
    	});
    	ids = ids.join(",");
